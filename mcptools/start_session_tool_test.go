...
func requireStartSessionResult(t *testing.T, result *mcputil.StartSessionResult, err error, opts startSessionResultOpts) {
	...
	payload := result.Payload.(*mcptools.StartSessionResult)
	require.NotNil(t, payload, "Result should not be nil")

	if opts.ShouldHaveConfig {
		assert.NotNil(t, payload.ServerConfig, "Server config should not be nil")
		assert.NotEmpty(t, payload.ServerConfig, "Server config should not be empty")

		// Verify server config contains expected fields
		assert.Contains(t, payload.ServerConfig, "serverName", "Config should contain server name")
		assert.Contains(t, payload.ServerConfig, "allowedPaths", "Config should contain allowed paths")
	}

	if opts.MinQuickStartItems > 0 {
		assert.GreaterOrEqual(t, len(payload.QuickStart), opts.MinQuickStartItems, "Should have minimum quick start items")
	}

	if opts.ShouldHaveGeneral {
		assert.NotEmpty(t, result.Instructions, "General instructions should not be empty")
	}

	if opts.MinLanguages > 0 {
		assert.GreaterOrEqual(t, len(payload.LanguageInstructions), opts.MinLanguages, "Should have minimum language instructions")
	}

	if opts.ShouldHaveToken {
		// Should have no current project when no paths are allowed
		assert.Nil(t, payload.CurrentProject, "Should have no current project when no paths allowed")
	}

	// Always check that extension mappings exist
	assert.NotNil(t, payload.ExtensionMappings, "Extension mappings should not be nil")
	assert.NotEmpty(t, payload.ExtensionMappings, "Extension mappings should not be empty")

	// Verify Go language instructions are present
	foundGo := false
	for _, lang := range payload.LanguageInstructions {
		if lang.Language == "go" {
			foundGo = true
	...
	assert.Equal(t, "go", payload.ExtensionMappings[".go"], "Should map .go to go")
	assert.Equal(t, "python", payload.ExtensionMappings[".py"], "Should map .py to python")
	assert.Equal(t, "javascript", payload.ExtensionMappings[".js"], "Should map .js to javascript")
...
func TestStartSessionTool(t *testing.T) {
	...
	t.Run("BasicSessionCreation_ShouldReturnValidSessionData", func(t *testing.T) {
		...
		mcputil.RegisterPayloadType(&mcptools.StartSessionResult{})
...
func TestToolMetadataConsistency(t *testing.T) {
	...
	for _, tool = range mcputil.RegisteredTools() {
		...
		if _, ok := tool.(*mcputil.StartSessionTool); !ok {
